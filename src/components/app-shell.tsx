...
export function AppShell({ children, sidebarExtra }: AppShellProps) {
  ...
  const [roles, setRoles] = useState<Role[]>(["unverified"]);
  ...
  useEffect(() => {
    ...
    const sessionRole = isDev ? "admin" : session?.role;
    const parsedRoles = parseRolesFromSession(sessionRole);

    // Admin cookie grants admin role regardless of session role
    if (!isDev && checkIsAdmin() && !parsedRoles.includes("admin")) {
      setRoles(["admin", ...parsedRoles]);
    } else {
