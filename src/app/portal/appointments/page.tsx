...
export default function AppointmentsPage() {
  ...
  const [allConsultations, setAllConsultations] = useState<Record<string, any>[]>([]);
  ...
  useEffect(() => {
    ...
    if (!id) return;
    setDataLoading(true);
    consultations.getByPatient(id)
      .then((list) => setAllConsultations(Array.isArray(list) ? list : []))
      .catch(() => setAllConsultations([]))
      .finally(() => setDataLoading(false));
  }, [patient]);

  const patientLoadingState = email !== null && patient === undefined;
  const dataLoadingState = patient != null && dataLoading;

  if (!sessionChecked || patientLoading || patientLoadingState || dataLoadingState) {
    return (
      <AppShell>
            ...
            <div className="glass-card">
              <div className="flex items-center gap-3 mb-5">
                ...
                <p className="text-sm font-medium text-foreground">
                  Provider Call &mdash; {formatPrice(SITECONFIG.billing.providerCallFee)}
                </p>
              </div>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-md px-4 py-3 mb-5">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-xs text-amber-800 leading-relaxed">
                    This is a {SITECONFIG.billing.providerCallDuration}-minute scheduled voice call. The provider may end the call at their discretion. Payment is non-refundable. There is no guarantee of a prescription.
                  </p>
                </div>
              </div>
          ...
          {upcoming.length > 0 ? (
            <div className="space-y-4">
              {upcoming.map((appt: any) => {
                ...
                return (
                  <div key={apptId} className="glass-card">
