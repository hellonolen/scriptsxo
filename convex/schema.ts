...
export default defineSchema({
  ...
  consultations: defineTable({
    ...
    providerId: v.optional(v.id("providers")), // optional for async_video intakes (assigned after provider approves)
  ...
  // === ASYNC VIDEO REVIEWS (AI-generated clinical summaries) ===
  videoReviews: defineTable({
    consultationId: v.id("consultations"),
    patientId: v.id("patients"),
    transcript: v.string(),
    summary: v.string(),
    ...
    recommendedAction: v.string(), // "approve" | "reject" | "needs_more_info"
    recommendationReason: v.string(),
    urgencyLevel: v.number(),
    confidence: v.number(),
    agentStatus: v.string(), // "pending" | "complete" | "error"
    // Provider decision
    providerDecision: v.optional(v.string()), // "approved" | "rejected"
    providerNotes: v.optional(v.string()),
    ...
    .index("by_status", ["agentStatus"]),

  // === PAPERCLIP-STYLE AGENT ORG LAYER ===

  // Ticket-based work queue — each unit of agentic work is a ticket
  agentTickets: defineTable({
    ticketId: v.string(),           // human-readable: "TKT-20240101-AB12CD"
    type: v.string(),               // "intake" | "triage" | "videoReview" | "prescription" | "pharmacy" | "notification" | "followUp" | "billing" | "marketing"
    status: v.string(),             // "queued" | "in_progress" | "complete" | "failed" | "escalated"
    priority: v.number(),           // 1 (lowest) - 5 (emergency)
    assignedAgent: v.string(),      // agent name e.g. "VideoReviewAgent"
    patientEmail: v.optional(v.string()),
    consultationId: v.optional(v.id("consultations")),
    intakeId: v.optional(v.id("intakes")),
    input: v.string(),              // JSON-serialized input data
    output: v.optional(v.string()), // JSON-serialized output data
    ...
    childTicketIds: v.optional(v.array(v.string())),
  })
    .index("by_status", ["status"])
  ...
  // Per-agent monthly token budget tracking
  agentBudgets: defineTable({
    agentName: v.string(),
    monthlyTokenBudget: v.number(),
    tokensUsedThisMonth: v.number(),
    alertThreshold: v.number(),     // 0.0 - 1.0 (0.8 = 80%)
    paused: v.boolean(),
  ...
  // Org chart definition — runtime-readable company structure
  ...
  // Company goals: mission -> projects -> tasks cascade
  companyGoals: defineTable({
    level: v.string(),              // "mission" | "project" | "task"
    title: v.string(),
    description: v.string(),
    ownerAgent: v.optional(v.string()),
    parentGoalId: v.optional(v.id("companyGoals")),
    status: v.string(),            // "active" | "complete" | "paused"
    metrics: v.optional(v.string()), // JSON: { target, current, unit }
  })
    .index("by_level", ["level"])
    .index("by_owner", ["ownerAgent"]),

  // === MARKETING CONTENT (generated by MarketingAgent) ===
  marketingContent: defineTable({
    type: v.string(), // "blog_post" | "google_ad" | "facebook_ad" | "email_subject" | "landing_page"
    topic: v.string(),
    targetKeyword: v.optional(v.string()),
    platform: v.optional(v.string()),
    content: v.string(), // full generated content (JSON string for structured types)
    status: v.string(), // "draft" | "approved" | "published" | "archived"
    generatedAt: v.number(),
    publishedAt: v.optional(v.number()),
    performance: v.optional(v.string()), // JSON: clicks, impressions, conversions
  })
    .index("by_type", ["type"])
    .index("by_status", ["status"]),
